...
@Component({
  selector: 'bandeja-documento-modales-agregar-usuario-distrib',
  templateUrl: 'agregar-usuario-distribucion.template.html',
  providers: [EquiposService, ColaboradoresService]
})
export class AgregarUsuarioDistribComponents implements OnInit {
...
//Habilitar Botón Limpiar 
  habilitarLimpiar(): void {
    ...
    if(this.funcion!='' ||this.responsable!=''||this.appaterno!='' || this.appmaterno!='' || this.plazo !=0 || this.prioridad !=0)
      this.deshabilitarLimpiar=false;
    else
      this.deshabilitarLimpiar=false;
  }    
