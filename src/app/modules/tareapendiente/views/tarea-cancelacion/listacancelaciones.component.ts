...
@Component({
  selector: 'app-listacancelaciones',
  templateUrl: 'listacancelaciones.template.html',
  styleUrls: ['listacancelaciones.component.scss'],
  //providers: [BandejaDocumentoService]
})
export class ListaCancelacionesComponent implements OnInit {

  /* codigo seleccionado */
  itemCodigo: number;
  listaTareasCancelaciones:any[];
  ...
  constructor(private localeService: BsLocaleService,
    ...
    this.selectedRow = -1;
    this.items = [];
    this.onCargarTareasCancelaciones();
  ...
  onCargarTareasCancelaciones(){
    this.listaTareasCancelaciones = [];
    this._jsonService.getTareasPendientesCanceladas().subscribe(
      response => {
        console.log("respuesta dashborad", response);
        this.listaTareasCancelaciones = response.responseData;
      },
      error => {
        alert("error "+ error);
      },
