...
import { BandejaDocumentoService, ParametrosService, CorreoService } from '../../../../services';
...
//import { Documento, RevisionDoumento } from 'src/app/models/documento';
...
@Component({
    selector: 'tareahomologacion-registro',
    templateUrl: 'tareahomologacion-registro.template.html',
    styleUrls: ['tareahomologacion-registro.component.scss']
})
export class TareaHomologacionRegistroComponent implements OnInit {
    ...
    mensaje: string;
    idFase: number;
    itemHomologacion: EnvioParametros;
    revisarDato: number;
    ...
    ngOnInit() {
        this.itemHomologacion = JSON.parse(sessionStorage.getItem("item"));

        this.itemCodigo = this.itemHomologacion.parametroPrincipal;
        this.idRevision = this.itemHomologacion.parametroSecundario;
    ...
    OnRegresar() {
        ...
        this.router.navigate([`documento/tareapendiente/HomologarRevision`]);
    }
    ...
    OnDocNvoGoogleDocs() {
        ...
        this.router.navigate([`documento/tareapendiente/NuevoDocGoogleDocsHomolog/${this.idRevision}/${this.itemCodigo}`]);
    }
    ...
    OnAprobar() {
        ...
        //Inicio Cambio Godar
        let tipoCorreo = 'NORMAL';
        if (documento.listaEquipo.length > 0) {
            for (let val of documento.listaEquipo) {
                if (val.descripcion == 'TODOS SEDAPAL') {
                    tipoCorreo = 'CORREO_SEDAPAL';
                    break;
                }
            }
                ...
                if (this.servicioValidacion.mapearErrores(errors, this.tab, this.mensajes)) {
                    ...
                    this.service.modificar(documento).subscribe(
                        (response: Response) => {
                            ...
                            } else if (tipoCorreo == 'CORREO_SEDAPAL') {
                                this.serviceCorreo.obtenerCorreo(documento.id, null,
                                    tipoCorreo).subscribe(
                                    (response: Response) => {
