...
@Component({
  selector: 'app-modal-observacion-programacion',
  templateUrl: './modal-observacion-programacion.component.html',
  styleUrls: ['./modal-observacion-programacion.component.scss'],
  providers: [ProgramacionAuditoriaService]
})
export class ModalObservacionProgramacionComponent implements OnInit {
...
/*     if(this.nuevo){
  ...
  agregar(){
    ...
    if (this.desObservacion == null || this.desObservacion.trim() == '' ) {
      this.desObservacion = null;
    } else {
      this.desObservacion = this.desObservacion;
    }

    this.bsModalRef.hide();
    this.onClose.next(this.desObservacion);
  }
    ...
    if(this.detalleProgramacionRegistro.listaNormas.length>0){
      ...
      for(let i:number=0; this.detalleProgramacionRegistro.listaNormas.length>i; i++){
        ...
        for(let j:number=0;normasQuitadas.length>j;j++){
          ...
          if(normaAux.idNorma == normaQuitadaAux.idNorma){
            listaItem.push(i);
            break;
          }
        }
    ...
    if(this.listaMeses.length>0){
      ...
      for(let i:number=0; this.listaMeses.length>i; i++){
        ...
        for(let j:number=0;mesesSeleccionados.length>j;j++){
          ...
          if(mesAux.v_campcons1 == mesSeleccionadoAux.v_campcons1){
            listaItem.push(i);
            break;
          }
        }
