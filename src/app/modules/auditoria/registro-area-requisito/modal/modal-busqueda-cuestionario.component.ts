...
@Component({
  selector: 'app-modal-busqueda-cuestionario',
  templateUrl: './modal-busqueda-cuestionario.component.html',
  styleUrls: ['./modal-busqueda-cuestionario.component.scss'],
  providers: [ProgramacionAuditoriaService]
})
export class ModalBusquedaCuestionarioComponent implements OnInit {
  ...
  obtenerParametros(){
    ...
    this.listaMeses = [];
    this.listaMeses = [
      { v_valcons: "4 Contexto de la Organización" },
      { v_valcons: "4.1 Comprension de la Organización y su Contexto" },
      { v_valcons: "4.2 Comprensión de las necesidades y expectativas de las partes interesadas" },
      { v_valcons: "4.3 Determinacion del Alcance del Sistema de Gestion de Calidad" },
      { v_valcons: "4.4 Sistema de Gestion de la Calidad y sus Procesos" },
      { v_valcons: "5 Liderazgo" },
      { v_valcons: "5.1 Liderazgo y Compromiso" },
      { v_valcons: "5.1.1 Generalidades" },
      { v_valcons: "5.1.2 Enfoque al Cliente" },
      { v_valcons: "5.2 Politica" },
      { v_valcons: "5.2.1 Establecimiento de la Politica de la Calidad" },
      { v_valcons: "6 Planificacion" },
      { v_valcons: "6.1 Acciones para abordar Riesgos y Oportunidades" },
      { v_valcons: "6.2 Objetivos de la Calidad y Planificacion para lograrlos" },
      { v_valcons: "6.3 Planificacion de los cambios" }
    ];
  ...
  crearListaDetalle(valorEntidad: any): Auditoria[] {
    ...
    for(let j:number = 0; this.detalleProgramacionRegistro.listaMeses.length > j; j++){
      ...
      auditoriaAux.mes = this.detalleProgramacionRegistro.listaMeses[j].v_campcons1;
    ...
    if (this.listaNormas.length > 0) {
      ...
      for(let i:number=0; this.listaNormas.length>i; i++){
        ...
        for (let j: number = 0; normasSeleccionadas.length > j; j++) {
          ...
          /*antes if(normaAux.idNorma == normaSeleccionadaAux.idNorma){
             listaItem.push(i);
             break;
           }*/
        }
