...
import { CursoAuditor } from 'src/app/models/interfaces/curso-auditor';
import { CursoNorma } from 'src/app/models/interfaces/curso-norma';
...
@Component({
  ...
  templateUrl: 'editar-ficha.template.html',
  styleUrls: ['editar-ficha.component.scss'],
  providers: [GeneralService, FichaRegistroAuditorService]
})

export class EditarRegistroAuditorNuevoComponent implements OnInit {
  ...
  private buscarIndiceCursoComp(nomCurso: string): number {
    let indice: number = -1;
    for (let i = 0; i < this.fichaAudiRequest.cursosCompletados.length; i++) {
      const element = this.fichaAudiRequest.cursosCompletados[i];
  ...
  private buscarIndiceCursoNorma(nomCurso: string): number {
    let indice: number = -1;
    for (let i = 0; i < this.fichaAudiRequest.cursosNorma.length; i++) {
      const element = this.fichaAudiRequest.cursosNorma[i];
  ...
  private buscarIndiceCursoPend(nomCurso: string): number {
    let indice: number = -1;
    for (let i = 0; i < this.fichaAudiRequest.cursosPendientes.length; i++) {
      const element = this.fichaAudiRequest.cursosPendientes[i];
  ...
  private cargarFichaSeleccionada(): void {
    this.fichaAudiRequest = StorageUtil.recuperarObjetoSession('fichaAuditorEditar');
  }

  private async enviarDatosApi() {
    this.loading = true;
    await this.fichaAuditorApi.actualizarFichaAuditor(this.fichaAudiRequest).toPromise()
      .then((response: Response) => {
        if (response.estado === 'OK') {
          this.loading = false;
          sessionStorage.setItem('flagRegFichaAudi', 'ACTUALIZADO');
  ...
  public getEnumFicha() {
    return ConstantesFichaAuditor;
  }

  private guardarFichaAudiExterno() {
    this.fichaAudiRequest.idUsuaModi = AgcUtil.camposAuditoria().idUsuaModi;
    this.enviarDatosApi();
  }
  ...
  public mostrarModalGuardar(): void {
    switch (this.fichaAudiRequest.tipoAuditor.codigo) {
  ...
  public onAgregarCursoComp(): void {
    if (AgcUtil.validarCampoTexto(this.cursoCompletadoAux)) {
      if (this.indObliCursoComp != null) {
        const indiceAux = this.buscarIndiceCursoComp(this.cursoCompletadoAux.trim());
        if (indiceAux === -1) {
          const cursoAuditor: CursoAuditor = {
            nomCurso: this.cursoCompletadoAux.trim(),
            indObli: this.indObliCursoComp
          };
          this.fichaAudiRequest.cursosCompletados.push(cursoAuditor);
        } else if (this.fichaAudiRequest.cursosCompletados[indiceAux].stReg) {
          if (this.fichaAudiRequest.cursosCompletados[indiceAux].stReg === ConstantesFichaAuditor.REG_ELIMINADO) {
            this.fichaAudiRequest.cursosCompletados[indiceAux].indObli = this.indObliCursoComp;
            this.fichaAudiRequest.cursosCompletados[indiceAux].stReg = ConstantesFichaAuditor.REG_ACTIVO;
          }
        } else {
  ...
  public onAgregarCursoNorma(): void {
    if (AgcUtil.validarCampoObjeto(this.cursoNormaAux)) {
      if (this.indObliCursoNorma != null) {
        const indiceAux = this.buscarIndiceCursoNorma(this.cursoNormaAux.descripcion);
        if (indiceAux === -1) {
          ...
          this.fichaAudiRequest.cursosNorma.push(cursoNorma);
        } else if (this.fichaAudiRequest.cursosNorma[indiceAux].stReg) {
          if (this.fichaAudiRequest.cursosNorma[indiceAux].stReg === ConstantesFichaAuditor.REG_ELIMINADO) {
            this.fichaAudiRequest.cursosNorma[indiceAux].indObli = this.indObliCursoNorma;
            this.fichaAudiRequest.cursosNorma[indiceAux].stReg = ConstantesFichaAuditor.REG_ACTIVO;
          }
        } else {
  ...
  public onAgregarCursoPend(): void {
    if (AgcUtil.validarCampoObjeto(this.cursoPendienteAux)) {
      if (this.indObliCursoPend != null) {
        const indiceAux = this.buscarIndiceCursoPend(this.cursoPendienteAux);
        if (indiceAux === -1) {
          const cursoPend: CursoAuditor = {
            nomCurso: this.cursoPendienteAux,
            indObli: this.indObliCursoPend
          };
          this.fichaAudiRequest.cursosPendientes.push(cursoPend);
        } else if (this.fichaAudiRequest.cursosPendientes[indiceAux].stReg) {
          if (this.fichaAudiRequest.cursosPendientes[indiceAux].stReg === ConstantesFichaAuditor.REG_ELIMINADO) {
            this.fichaAudiRequest.cursosPendientes[indiceAux].indObli = this.indObliCursoComp;
            this.fichaAudiRequest.cursosPendientes[indiceAux].stReg = ConstantesFichaAuditor.REG_ACTIVO;
          }
        } else {
          this.toastrUtil.showWarning('El curso ingresado ya se encuentra en la lista');
        }
  ...
  public onEliminarCursoComp(): void {
    if (this.selectedCursoComp !== null) {
      ...
      const curso: CursoAuditor = this.fichaAudiRequest.cursosCompletados[indiceEliminar];
      if (curso.idCurso !== null && curso.idCurso !== undefined) {
        this.fichaAudiRequest.cursosCompletados[indiceEliminar].stReg = ConstantesFichaAuditor.REG_ELIMINADO;
      } else {
        this.fichaAudiRequest.cursosCompletados.splice(indiceEliminar, 1);
      }
  ...
  public onEliminarCursoNorma(): void {
    if (this.selectedCursoNorma !== null) {
      ...
      const curso: CursoNorma = this.fichaAudiRequest.cursosNorma[indiceEliminar];
      if (curso.idCursoNorma !== null && curso.idCursoNorma !== undefined) {
        this.fichaAudiRequest.cursosNorma[indiceEliminar].stReg = ConstantesFichaAuditor.REG_ELIMINADO;
      } else {
        this.fichaAudiRequest.cursosNorma.splice(indiceEliminar, 1);
      }
      this.limpiarRowCursoNorma();
    }
  }
  ...
  public onEliminarCursoPend(): void {
    if (this.selectedCursoPend !== null) {
      ...
      const curso: CursoAuditor = this.fichaAudiRequest.cursosPendientes[indiceEliminar];
      if (curso.idCurso !== null && curso.idCurso !== undefined) {
        this.fichaAudiRequest.cursosPendientes[indiceEliminar].stReg = ConstantesFichaAuditor.REG_ELIMINADO;
      } else {
        this.fichaAudiRequest.cursosPendientes.splice(indiceEliminar, 1);
      }
  ...
  public validarGuardarAudiInterno(): void {
    if (!AgcUtil.validarCampoObjeto(this.fichaAudiRequest.rolAuditor)) {
      this.listaMensaje.push('Por favor, seleccione un Rol');
    }
    if (!AgcUtil.validarCampoObjeto(this.fichaAudiRequest.tipoEducacion)) {
      this.listaMensaje.push('Por favor, seleccione el tipo de Educacion');
    }
    if (this.fichaAudiRequest.cursosNorma.length === 0) {
      this.listaMensaje.push('Por favor, ingrese normas en la pestaña cursos');
    }
    if (this.fichaAudiRequest.cursosCompletados.length === 0) {
      this.listaMensaje.push('Por favor, ingrese los cursos llevados, en la pestaña cursos');
    }
    if (this.fichaAudiRequest.cursosPendientes.length === 0) {
