...
@Component({
  selector: 'app-registrar-deteccion-fuera',
  templateUrl: './registrar-deteccion-fuera.component.html',
  styleUrls: ['./registrar-deteccion-fuera.component.scss'],
  providers: [DeteccionHallazgosService]
})
export class RegistrarDeteccionFueraComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.listaTipoNoConformidad = [];
    this.listaNormas = [];
    this.mostrarComentario = false;
    this.desactivaIncidencia = true;
    ...
    this.sub = this.route.params.subscribe(params => {
      ...
      if (this.itemCodigo) {
        ...
        if (tipoDet == "2") {
          this.deteccionHallazgos.ambito = "Fuera del SGI"
          this.deteccionHallazgos.valorAmbito = "2";
        }
        let origenDeteccion = localStorage.getItem("origenDet");
        //let datoOrigen=this.listaIncidenciasCombo.find(obj=> obj.idIncidencia==origenDeteccion);
    ...
    /*
        ...
        if(tipoDet=="2"){
          this.deteccionHallazgos.tipo="Fuera del SGI"
        }  
        let origenDeteccion =localStorage.getItem("origenDet");
     
        let datoOrigen=this.listaIncidenciasCombo.find(obj=> obj.idIncidencia==origenDeteccion);
        this.deteccionHallazgos.origenDeteccion=datoOrigen.descripcionIncidencia;
     */
  ...
  obtenerParametrosInc() {
    ...
    const buscaNormas = this.normaService.obtenerNormas("2");
  ...
  llenarArbol() {
    ...
    this.planAuditoriaService.buscarIncidencias("1").subscribe(
      (response: Response) => {
  ...
  */
  ...
  obtenerRequisitosIncidencia(idNicidencia: string) {
    this.loading = true;
    this.normaService.buscarRequisitosNorma(idNicidencia, "2").subscribe(
      (response: Response) => {
  ...
  cambiaIncidencia() {
    ...
    if (norma != "") {
      ...
      this.deteccionHallazgos.descripHallazgo = "";
      this.mostrarComentario = false;
      this.desactivaIncidencia = false;
      this.obtenerRequisitosIncidencia(norma);
    } else {
      this.listaRequisitos = [];
      this.listaDestinoTreeNode = [];
      this.listaDestino = [];
